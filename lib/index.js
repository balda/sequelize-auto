...
AutoSequelize.prototype.run = function(options, callback) {
  ...
  options.interspace = options.interspace || ' ';
  ...
  function generateText(err) {
    ...
    async.each(tableNames, function(table, _callback){
      ...
      text[table] += "module.exports = function(" + options.interspace + "sequelize, DataTypes" + options.interspace + ") {\n";
      text[table] += spaces + "return sequelize.define(" + options.interspace + options.quote + table + options.quote + ", {\n";
      ...
      _.each(fields, function(field, i){
        ...
        if (_tables[table][field].type === "USER-DEFINED" && !! _tables[table][field].special) {
          _tables[table][field].type = "ENUM(" + _tables[table][field].special.map(function(f){ return options.quote + f + options.quote; }).join(','+options.interspace) + ")";
        }
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              text[table] += spaces + spaces + spaces + "autoIncrement: true";
                ...
                console.log( table + " : " + field + " : " + attr );
                    console.log( _tables[table][field][attr] );
              text[table] += spaces + spaces + spaces + "references: {\n";
              text[table] += spaces + spaces + spaces + spaces + "model: " + options.quote + _tables[table][field][attr].target_table + options.quote + ",\n"
              text[table] += spaces + spaces + spaces + spaces + "key: " + options.quote + _tables[table][field][attr].target_column + options.quote + "\n"
            ...
            if (_.isString(defaultVal)) {
              val_text = options.quote + val_text + options.quote
            }
            if(defaultVal === null) {
              return true;
            } else {
            ...
            text[table] += spaces + spaces + spaces + attr + ": DataTypes." + _tables[table][field][attr].split( "'" ).join( options.quote );
          } else {
            var _attr = (_tables[table][field][attr] || '').toLowerCase();
            var val = options.quote + _tables[table][field][attr] + options.quote;
      ...
      text[table] += spaces + spaces  + "tableName: " + options.quote + table + options.quote + ",\n";
